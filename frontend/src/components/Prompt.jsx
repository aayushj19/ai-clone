...
function Prompt() {
  ...
  useEffect(() => {
    ...
    if (user) {
      const storedPromt = localStorage.getItem(`promptHistory_${user._id}`);
      if (storedPromt) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        "http://localhost:5000/api/v1/ai/prompt",
        { content: trimmed },
        {
          headers: {
  ...
  return (
    <div className="flex flex-col items-center justify-between flex-1 w-full px-4 pb-4 ">
      ...
      {/* ➤ Scrollable Chat Box */}
      <div className="w-full max-w-4xl flex-1 overflow-y-auto pt-2 space-y-4 max-h-[50vh] px-1">
        {promt.map((msg, index) => (
          <div
            key={index}
                  ...
                  components={{
                    code({ node, inline, className, children, ...props }) {
                      ...
                      return !inline && match ? (
                        ...
                        <code
                          className="bg-gray-800 px-1 py-1 rounded-full"
                          {...props}
                        >
                          {children}
                        </code>
        ...
        {loading && typeMessage && (
          <div
            className="whitespace-pre-wrap px-4 mb-0 py-3 rounded-2xl text-sm break-words
      ...
      {/* ➤ Input Box */}
      <div className="w-full max-w-3xl relative mt-auto">
